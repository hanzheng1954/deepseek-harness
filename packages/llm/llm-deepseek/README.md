...
`queryBalance('deepseek-official')` is the adapter's account-balance capability on the generic seam: it reads `GET {baseURL}/user/balance` through the same per-operation connection/credential resolution as `stream` — the endpoint and bearer key come from one snapshot, so a key can never be sent to an endpoint from another configuration generation — and carries the same attribution headers. A 200 whose `is_available` is false, or an account with no balance rows, answers `undefined` (the account-less case, never an error); non-2xx responses throw the same `LlmError` code taxonomy as model calls (`AUTH` for 401/403, `SERVER` for 5xx, …) with the HTTP status on `failure`, and pre-response transport failures throw `TRANSPORT`. Adapters whose providers expose no such endpoint simply omit the method.
