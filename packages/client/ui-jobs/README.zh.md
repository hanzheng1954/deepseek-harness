...
存在活跃后台任务的 Web GUI 会话会显示一个头部动作，用于打开其可见任务列表。运行时提供宿主计算的 `jobsBySession`；本包不发 RPC。角标计数运行中与停止中的任务。只要还有其他活跃工作，终态行就继续显示；最后一个活跃任务结束时，控件会先关闭再卸载，因为持久结果属于会话中的工具行。`dsh-tool-jobs` 向模型提供独立视角；本包是给人类看的只读投影。
