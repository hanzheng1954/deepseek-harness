...
The third entry — `deepseek-balance` in the `conversation.composer.dock` stats band — is an ambient, non-interactive readout: `llm.balance` resolves the DeepSeek official account balance host-side (the injected `loadBalance` carries it into the framework-free `BalanceChip`), and the chip polls once a minute plus on window focus. An unavailable or capability-less account renders nothing; a failed query degrades to an error label whose hover detail carries the Host diagnosis. The dock slot contract keeps clickable controls in the tool row, so the chip is deliberately not one.
