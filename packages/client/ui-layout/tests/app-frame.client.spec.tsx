...
describe('AppFrame', () => {
  ...
  it('renders owner props for the default sidebar and prospective right panel', () => {
    ...
    expect(sidebarOwner()).toEqual({ collapsed: false, width: 280, narrow: false })
  ...
  it('keeps Windows caption controls mounted with a zero-width collapsed column', () => {
    ...
    try {
      ...
      expect(sidebarOwner()).toMatchObject({ collapsed: true, width: 0, narrow: false })
      expect(getByTestId('sidebar-content')).toBeTruthy()
    } finally {
  ...
  it('keeps the closed sidebar mounted at its 56px rail without a handle', () => {
    ...
    expect(sidebarOwner()).toEqual({ collapsed: true, width: 56, narrow: false })
    expect(getByTestId('sidebar-content')).toBeTruthy()
...
describe('AppFrame normal width concessions', () => {
  ...
  it('uses the post-dismissal left rail to permit a narrow first opening', () => {
    frameWidth = 800
    ...
    expect(frame.querySelector('[data-side="sidebar"]')).toBeNull()
    expect(sidebarOwner()).toEqual({ collapsed: false, width: 280, narrow: true })
